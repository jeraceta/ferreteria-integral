...
router.get("/reporte-ganancias", esAdmin, async (req, res, next) => {
  try {
...
router.get("/reporte-comisiones", esAdmin, async (req, res, next) => {
  try {
...
router.post("/producto", requiereAuth, esAdmin, async (req, res, next) => {
  // Combined HEAD and 8880b31
...
router.post("/compra", esAdmin, async (req, res, next) => {
  // Combined HEAD and 8880b31
  ...
  try {
    ...
    res
      .status(201)
      .json({ mensaje: "Compra procesada", compraId: resultado.id_compra });
  } catch (error) {
    next(error);
  }
});
...
router.post("/traslado", requiereAuth, esAdmin, async (req, res, next) => {
  try {
    const { id_producto, id_origen, id_destino, cantidad, comentario } =
      req.body;

    await trasladarMercancia(
      id_producto,
      id_origen,
      id_destino,
      cantidad,
...
//router.get('/reporte-x', requiereAuth, esAdmin, obtenerReporteX);

// Reporte Z: Cierre definitivo de caja

//router.post('/reporte-z', requiereAuth, esAdmin, generarCierreZ);
...
router.get("/ganancias", requiereAuth, esAdmin, async (req, res, next) => {
  try {
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const data = await obtenerVentasPorVendedor(
        fechaInicio,
        fechaFin,
        porcentajeComision,
      );
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const granTotal = valoracion.reduce(
        (acc, cat) => acc + parseFloat(cat.inversion_total_costo || 0),
        0,
      );

      res.json({
        success: true,
        gran_total_inventario: granTotal,
        data: valoracion,
      });
    } catch (error) {
      next(error);
...
router.get("/usuarios", requiereAuth, esAdmin, async (req, res, next) => {
  try {
...
router.put("/usuarios/:id", esAdmin, async (req, res, next) => {
  try {
...
router.delete("/usuarios/:id", esAdmin, async (req, res, next) => {
  try {
