...
router.get(
  "/categorias",
  requiereAuth,
  inventarioController.obtenerTodasLasCategorias,
);
...
router.get(
  "/buscar",
  requiereAuth,
  inventarioController.buscarProductosPredictivo,
);
...
// Los vendedores pueden editar productos pero no precios ni stock.
router.put("/:id", requiereAuth, async (req, res, next) => {
  try {
    await inventarioController.actualizarProducto(req, req.params.id, req.body);
